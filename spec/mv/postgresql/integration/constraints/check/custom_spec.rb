...
CustomCheckTestTableName = Class.new(ActiveRecord::Base) do
...
describe "custom validation in trigger constraint begaviour" do
    ...
    Class.new(::ActiveRecord::Migration[5.0]) do
      def change
        create_table :table_name, id: false do |t|
          t.integer :custom, validates: { custom: { statement: '{custom} > 1', allow_nil: true, as: :check } }
        end
      end
  ...
  subject(:insert) { CustomCheckTestTableName.create! opts }
