...
LengthCheckTestTableName = Class.new(ActiveRecord::Base) do
  self.table_name = :table_name
end

describe "length validation in check constraint begaviour" do
    ...
    Class.new(::ActiveRecord::Migration) do
      def change
        create_table :table_name, id: false do |t|
          ...
          t.string :length_in_array, validates: { length: { in: [1, 5], allow_nil: true, as: :check } }
          t.string :length_in_range, validates: { length: { in: 1..5, allow_nil: true, as: :check } }
          t.string :length_within_array, validates: { length: { within: [1, 5], allow_nil: true, as: :check } }
          t.string :length_within_range, validates: { length: { within: 1..5, allow_nil: true, as: :check } }
          t.string :length_minimum, validates: { length: { minimum: 5, allow_nil: true, as: :check } }
          t.string :length_maximum, validates: { length: { maximum: 5, allow_nil: true, as: :check } }
        end
      end
  ...
  subject(:insert) { LengthCheckTestTableName.create! opts }
