...
=== uniqueness
  ...
  Options: 
     ...
     <b>message:</b>   text of the error message that will be shown if constraint violated.  Ignored unless :as == :trigger
     index_name:   name of the index that will be created for validator. Ignored unless :as == :index
     on:   
     as: defines the way how constraint will be implemented. Possible values: [:index, :trigger]
=== length
=== inclusion
=== exclusion
=== presense
